...
export class PbUtils {
  ...
  public static async getMessages(channelId: string): Promise<Message[]> {
    return await pb.collection(Collections.MESSAGES).getFullList<Message>({
    ...
    return pb
      ...
      .then((channels) => {
        channels.forEach((channel) => {
          if (
            (channel.name.includes("-" + currentUsername) ||
              channel.name.includes(currentUsername + "-")) &&
            !channel.isPublic
          ) {
  ...
  public static async getUserProfile(): Promise<UserProfile> {
    return (await pb.collection<UserProfile>(Collections.USERS).authRefresh())
      .record;
  }
      ...
      .then((channel) => {
        ...
        for (let i = 0; i < channel.users.length; i++) {
          filterStr +=
            ...
            channel.users[i] +
            "'" +
            (i === channel.users.length - 1 ? "" : " || ");
        }

        console.log("Filter string:", filterStr);

        return pb
          .collection(Collections.USERS_OVERVIEW)
          .getFullList<Member>({
            filter: filterStr,
          })
          .then((result) => {
